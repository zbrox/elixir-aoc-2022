defmodule Aoc.Y2022.Day07 do
  ...
  *What is the total size of that directory?*
  ...
  defmodule State do
    defstruct cwd: [], files: [], dirs: [%Dir{path: "/"}]
  end

  def parse_line("$ cd " <> to_dir, state) do
    case to_dir do
      "/" -> state |> Map.put(:cwd, [])
      ".." -> state |> Map.put(:cwd, List.pop_at(state.cwd, -1) |> elem(1))
      v -> state |> Map.put(:cwd, state.cwd ++ [v])
    end
  end

  def parse_line("$ ls", state) do
    state
  end
  ...
  def parse_line(line, state) do
    case String.split(line, " ") do
      ["dir", dir_name] ->
        ...
        |> Map.put(
          :dirs,
          state.dirs ++ [%Dir{path: create_path(state.cwd, dir_name)}]
        )
      ...
      [size, file_name] ->
        ...
        |> Map.put(
          :files,
          state.files ++ [%File{path: create_path(state.cwd, file_name), size: String.to_integer(size)}]
        )
    end
  end

  def create_path(cwd \\ [], filename \\ "") do
    path = "/" <> Enum.join(cwd, "/") <> "/" <> filename
    case path do
      "//" -> "/"
      _ -> String.replace_leading(path, "//", "/")
    end
  end

  def calculate_sizes(dirs, files) do
    dirs
    |> Enum.map(fn dir ->
      {dir.path, dir_size(dir.path, files)}
    end)
  end

  def dir_size(dir_path, files) do
    files
    |> Enum.filter(fn file -> String.starts_with?(file.path, dir_path) end)
  ...
  def part1(input) do
    ...
    calculate_sizes(state.dirs, state.files)
